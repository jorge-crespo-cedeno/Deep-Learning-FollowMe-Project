...
The neural network design can be used to follow another object, such as a dog, cat, car, because the fully convolutional neural network will manage to extract the required features to characterize an object. The first layer can extract basic features, and the following layers can build upon them to characterize a complex object. The up-sampling and fully connected layers will be able to provide localization and classification information as well. But first, an appropriate model needs to be generated through training and validation of the neural network with large set of images. They must contain the target (cat, dog, etc) to be followed, and also some other objects that are not the target, so that the model can learn to discriminate between target and non-target objects. Also, the data should be captured in scenarios similar to the ones used in this project, where the quadrotor is following the target or in patrol mode, sometimes including the target and sometimes not for the latter case.
